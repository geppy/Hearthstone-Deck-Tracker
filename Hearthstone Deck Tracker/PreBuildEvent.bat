...
if exist "%SolutionDir%HearthSim.Common" (
  echo Updating "%SolutionDir%HearthSim.Common" to origin/master
  git -C "%SolutionDir%HearthSim.Common" fetch
  git -C "%SolutionDir%HearthSim.Common" reset --hard origin/master
) else (
  git clone --depth 1 https://github.com/HearthSim/HearthSim.Common.git "%SolutionDir%HearthSim.Common"
)
